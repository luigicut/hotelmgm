...
/**
 * Questa classe definisce un oggetto FrameElencoUtente
 * @author Daniele
 *
 */
public class FrameElencoUtente extends JFrame{
	 ...
	 * @param ArrayList<Account> lista degli account
	 */
		public FrameElencoUtente(Contenitore c,ArrayList<Account> a){
			...
			BarraMenuIS barra=new BarraMenuIS(frame,contenitore,6);
			...
			JLabel larea=new JLabel("                 Elenco Utenti  ");
			larea.setFont(font);
	...
	String[] nomiColonne={"Nome Utente","Password","Nome","Cognome","Data Nascita","Ruolo"};
	String[][] celle=new String[l][6];
		
	 for(Account acc:array){
		celle[indice][0]=acc.getLogin();
	    celle[indice][1]=acc.getPassword();
		celle[indice][2]=acc.getNome();
		celle[indice][3]=acc.getCognome();
		celle[indice][4]=acc.getDataNascita();
		celle[indice][5]=acc.getRuolo();
		indice++;
	}
