...
/**
 * Questa classe definisce un oggetto FrameRisultatiAggiungiCliente
 * @author Daniele
 *
 */
public class FrameRisultatiAggiungiCliente extends JFrame{
	 ...
	 * @param ArrayList<Clienti> lista dei clienti
	 */
		public FrameRisultatiAggiungiCliente(Contenitore c,ArrayList<Cliente> arr){
	...
	class addActionListenerAvanti implements ActionListener
	{
		public void actionPerformed(ActionEvent event){
			...
			boolean flag=false;
		    int codiceCliente=Integer.parseInt(areaIns.getText());
		    
		    for(Cliente client:arrayCli){
		    	int v=client.getCodice();
		    	if (v==codiceCliente){flag=true;break;}
		    }
		    if (!flag) {throw new ErrorException(contenitore,"Codice non presente nell'elenco! Riprovare");}
		    else{		
			FrameAggiungiPersone p=new FrameAggiungiPersone(contenitore,codiceCliente);
			p.visualizzaFrame();
	...
	JLabel ins=new JLabel("      Inserire il codice del cliente a cui associare le persone: ");
