...
/**
 * Questa classe definisce un oggetto FrameElencoPersone
 * @author Daniele
 *
 */
public class FrameElencoPersone extends JFrame{
	 ...
	 * @param ArrayList<Persona> lista delle persone
	 */
		public FrameElencoPersone(Contenitore c,ArrayList<Persona> arr){
			...
			BarraMenuIS barra=new BarraMenuIS(frame,contenitore,17);
			...
			JLabel larea=new JLabel("    Elenco Persone Associate");
			larea.setFont(font);
	...
	String[] nomiColonne={"Codice Persona Associata","Nome","Cognome","Data Nascita","Luogo di Nascita","Provincia"};
	
	String[][] celle=new String[l][6];
	
	 for(Persona per:arrayP){
		celle[indice][0]=Integer.toString(per.getCodice());
	    celle[indice][1]=per.getNome();
		celle[indice][2]=per.getCognome();
		celle[indice][3]=per.getDataNascita();
	    celle[indice][4]=per.getLuogoNascita();
		celle[indice][5]=per.getProvincia();
